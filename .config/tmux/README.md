...
- **左**：状態表示＋window一覧。色だけでなく文字でも分かるようにしてある。複数の状態が同時に起きても重ならないよう優先順位で1つだけ表示する
  ...
  - window名はデフォルトのautomatic-renameで実行中コマンド名になるが、Claude Codeは自身のプロセス名をバージョン文字列（例: `2.1.219`）に書き換えているため、`automatic-rename-format`で数字.数字.数字だけの名前を`claude`に読み替えている（`tmux.conf`参照）
