...
| Caskパッケージ | 用途 | 必須/任意 |
|---|---|---|
| [font-roboto-mono-nerd-font](https://github.com/ryanoasis/nerd-fonts) | `config.ghostty`の`font-family`で使うNerd Fontフォント | 任意（Ghosttyを使う場合のみ必須） |
...
既存の `~/.config/ghostty/config` は上書きしてリンクを張る。

```bash
rm -f ~/.config/ghostty/config
mkdir -p ~/.config/ghostty
ln -s ~/codes/datfile/config.ghostty ~/.config/ghostty/config
```

前述の「Homebrewでインストールするもの」の`font-roboto-mono-nerd-font`を事前にインストールしておく。
反映するにはGhosttyを再起動する。
